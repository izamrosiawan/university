...
const COLORREF CaptionInvisible = DWMWA_COLOR_NONE;  // Match border - transparent
...
HRESULT WINAPI DwmSetWindowAttribute_hook(HWND hwnd, DWORD dwAttribute, LPCVOID pvAttribute, DWORD cbAttribute) {
    if ((dwAttribute == DWMWA_BORDER_COLOR || dwAttribute == DWMWA_CAPTION_COLOR) && IsValidWindow(hwnd)) {
        Wh_Log(L"Intercepted DWM attribute %lu - setting to invisible", dwAttribute);
        COLORREF invisibleColor = (dwAttribute == DWMWA_CAPTION_COLOR) ? CaptionInvisible : BorderInvisible;
        return DwmSetWindowAttribute_orig(hwnd, dwAttribute, &invisibleColor, sizeof(invisibleColor));
    }
...
void SetBorderInvisible(HWND hWnd)
{
    ...
    Wh_Log(L"Setting invisible border (Win11)");
    DwmSetWindowAttribute_orig(hWnd, DWMWA_BORDER_COLOR, &BorderInvisible, sizeof(BorderInvisible));
    DwmSetWindowAttribute_orig(hWnd, DWMWA_CAPTION_COLOR, &CaptionInvisible, sizeof(CaptionInvisible));
}
...
BOOL CALLBACK EnableEnumWindowsCallback(HWND hWnd, LPARAM lParam) {
    ...
    // If pid is 0, apply to all windows; otherwise filter by PID
    if (pid == 0 || pid == wPid) {
        SetBorderInvisible(hWnd);
    }
...
BOOL CALLBACK DisableEnumWindowsCallback(HWND hWnd, LPARAM lParam) {
    ...
    // If pid is 0, apply to all windows; otherwise filter by PID
    if ((pid == 0 || pid == wPid) && IsValidWindow(hWnd))
...
void Wh_ModAfterInit() {
    ...
    EnumWindows(EnableEnumWindowsCallback, 0);
}

void Wh_ModBeforeUninit() {
    Wh_Log(L"BeforeUninit - Restoring default borders");
    EnumWindows(DisableEnumWindowsCallback, 0);
}
